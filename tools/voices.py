...
# The grain shared by every inner voice: saturation for rasp, a low shelf for
# chest, a presence lift so it cuts through the music, then compression.
def grit(drive, fry, chest=5, presence=3):
    parts = [
        'highpass=f=60',
        'equalizer=f=120:t=q:w=0.9:g=%g' % chest,
        'equalizer=f=320:t=q:w=1.2:g=-2',
        'equalizer=f=2800:t=q:w=1.4:g=%g' % presence,
        'volume=%gdB' % drive,
        'asoftclip=type=atan:oversample=2',
        'volume=-%gdB' % (drive * 0.8),
    ]
    if fry:
        parts.append('tremolo=f=34:d=%g' % fry)
    parts.append('acompressor=threshold=-22dB:ratio=4:attack=4:release=140:makeup=3')
    return parts
...
PROFILES = {
    ...
    'REASON': dict(voice='en_US-norman-medium', length=1.06, pitch=0.87,
                   chain=grit(9, 0.10, chest=4, presence=4) + room([30, 55], [0.10, 0.06])),
    'SOUL': dict(voice='en_US-norman-medium', length=1.16, pitch=0.84,
                 chain=grit(7, 0.12, chest=6, presence=2) + room([45, 90, 150], [0.16, 0.11, 0.06])),
    'FLESH': dict(voice='en_US-norman-medium', length=1.08, pitch=0.81,
                  chain=grit(13, 0.2, chest=7, presence=3) + room([25, 50], [0.08, 0.05])),
    'NERVE': dict(voice='en_US-norman-medium', length=1.03, pitch=0.86,
                  chain=grit(10, 0.12, chest=5, presence=4) + room([30, 60], [0.09, 0.06])),
    # The cold under the ice: slower, lower, far away.
    'THE COLD': dict(voice='en_US-norman-medium', length=1.45, pitch=0.72,
                     chain=grit(8, 0.25, chest=6, presence=0) + ['lowpass=f=2400'] + room([120, 260, 420], [0.35, 0.25, 0.15], 0.8, 0.6)),
    ...
    'SARRE': dict(voice='en_GB-northern_english_male-medium', length=1.12, pitch=0.9,
                  chain=grit(8, 0.15, chest=5, presence=1) + ['lowpass=f=3600'] + room([70, 150, 240], [0.28, 0.18, 0.1], 0.8, 0.65)),
...
def main():
    ...
    for fname, group in groups:
        ...
        def flush():
            ...
            subprocess.run([FFMPEG, '-hide_banner', '-loglevel', 'error', '-y', '-i', tmp, '-c:a', 'libmp3lame', '-b:a', '40k', '-ar', str(RATE), '-ac', '1', os.path.join(OUT_DIR, name)], check=True)
            fi = len(files)
